...
pthread_t client0, client1, client2, client3, client4, client5, client6, client7, client8, client9;
vector<pthread_t> client = {
    client0,
    client1,
    client2,
    client3,
...
ThreadData data0, data1, data2, data3, data4, data5, data6, data7, data8, data9;
...
void produce(string input) {
    ...
    item = make_pair(int(gettid()), input);
...
void *thread_(void *t_ptr) {
    ThreadData *l_ptr = (ThreadData*)t_ptr;

    int l_sock_client = l_ptr->fileDesc;
    ...
    { // communication
        if (firstConsume && spotreba) {
            pair<int, string> item = consume();         
            firstConsume = 0;

            string msg = "TID: " + to_string(item.first) + ", item: " + item.second + "\n";
            write( l_sock_client, msg.c_str(), strlen(msg.c_str()) );
        }
...
/*
Server je na maximálně 10 klientů, u připojeného klienta je po spuštění
potřeba napsat "VYROBA" nebo "SPOTREBA".
*/
                if (current < 10) {
                    data[current] = {l_sock_client};
                    pthread_create(&client[current], NULL, thread_, &data[current]);
                    current++;
                    continue;
                } else {
                    log_msg( LOG_ERROR, "Unable to accept new client." );
